...
Note: This document uses ROLL as the concrete reference for the control-plane shape. The shared, cross-framework protocol (checkpoint versioning, elastic shrink/expand,
and request/trajectory migration) is defined in `design_doc/multi-pipeline-adaptation-plan_clean.md`.
...
- 2) Architecture: decouple GPU scheduling from pipeline coordination
  ...
  - Coordination protocol:
    - Training clusters (trainer-side compute: train/value/logprobs): execution is driven by the pipeline coordinator (request → run → `release_gpus()`),
      ...
      - Release ordering: the coordinator releases the training cluster locally (stop/offload workers) and then calls `release_gpus()` as a notification so the
        centralized scheduler can update global accounting and unblock future allocations.
    - Generation cluster (generator-side compute: `actor_infer` rollouts): preemptible DP workers and completion-driven release:
      ...
      - After each training step, the pipeline coordinator requests the centralized scheduler to release generation capacity; the scheduler performs the actual shrink-to-zero
        (or partial shrink) of generation DP workers so shrink/expand concurrency is globally coordinated across pipelines.
  ...
  - Workflow diagram
    ...
    sequenceDiagram
      ...
      RCP->>W: release training cluster GPUs (stop/offload)
      RCP-->>CGS: release_gpus(training cluster) (notification after release)

      rect rgb(235, 245, 255)
        Note over RCP,CGS: Coordinator-initiated start (request + initial resume)
        RCP->>CGS: request_gpus(generation cluster (actor_infer), generation/rollout)
        CGS->>W: resume DP ranks via expand_workers(...) (preemptible)
      end
      W->>W: (re)activate DP ranks
      W->>W: run rollout

      rect rgb(245, 245, 245)
        Note over CGS,W: Scheduler-driven time-sharing resize (while rollout is active)
        loop Time-sharing resize
          CGS->>W: preempt DP ranks via shrink_workers(...)
          CGS->>W: resume DP ranks via expand_workers(...)
          W->>W: reroute/retry and continue rollout
        end
      end

      rect rgb(235, 245, 255)
        Note over RCP,CGS: Coordinator-initiated end (release request)
        RCP->>CGS: request_release_gpus(generation cluster (actor_infer))
        CGS->>W: shrink DP ranks (possibly to zero) via shrink_workers(...)
        W->>CGS: notify_cluster_released() (blocking ACK)
        CGS-->>RCP: release_complete / allocation_applied
      end
      Note over RCP,W: After release_complete, coordinator may perform model update/refit (QUIESCE) before next resume
    ```
